library(dplyr)
library(Strategus)
rootFolder <- "C:/Users/fanr/Desktop/Project/semaglutideNvamd/SemaglutideNvamd"
...
studyEndDate <- '20241231'   #YYYYMMDD # changed study end date
# This is lame but has to be done
studyStartDateWithHyphens <- '2017-12-01' #YYYYMMDD
studyEndDateWithHyphens <- '2024-12-31'   #YYYYMMDD # changed study end date
...
# simplified NCO data frame input codes since column names were already specified 
...
cohortGeneratorModuleSpecifications <- cgModuleSettingsCreator$createModuleSpecifications(
  # incremental = TRUE, # don't need it for the new Strategus version
  generateStats = TRUE
)
...
characterizationModuleSpecifications <- cModuleSettingsCreator$createModuleSpecifications(
  ...
  outcomeWashoutDays = oList$cleanWindow, # added outcomeWashoutDays parameter for new Strategus verion. Removed timeAtRisk parameter
...
irDesign <- CohortIncidence::createIncidenceDesign(
  ...
  strataSettings = CohortIncidence::createStrataSettings(
    ...
    byGender = TRUE,
    byAge = TRUE,
    ageBreaks = seq(0, 110, by = 5) # changed age breaks from 10 to 5
  )
)
...
cohortMethodModuleSpecifications <- cmModuleSettingsCreator$createModuleSpecifications(
  ...
  cmDiagnosticThresholds = CohortMethod::createCmDiagnosticThresholds(
    mdrrThreshold = 10, # changed mdrrThreshold from inf to 10 
    easeThreshold = 0.25,
    sdmThreshold = 0.1,
...
for (i in seq_len(nrow(sccsIList))) {
  indicationId <- sccsIList$targetCohortId[i] # changed the column name from `indicationCohortId` to `targetCohortId`
