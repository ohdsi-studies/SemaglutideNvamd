...
# Upload Results ---------------------------------------------------------------
resultsFolders <- list.dirs(path = "results", full.names = T, recursive = F)
#resultsFolders <- c("results/washu", "results/jhu", "results/usc")
for (resultFolder in resultsFolders) {
  resultsDataModelSettings <- Strategus::createResultsDataModelSettings(
    resultsDatabaseSchema = resultsDatabaseSchema,
    resultsFolder = file.path(resultFolder, "strategusOutput"),
  )
  cli::cli_alert_info(paste0("Uploading ", resultFolder))
